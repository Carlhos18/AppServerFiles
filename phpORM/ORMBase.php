	...
	{		
         ...
         */
          public function combo_orm($id,$descripcion,$table_orm,$pk_orm,$field,$estate=true,$condition=array()){
              ...
              include_once "models/$table_orm.php";
          ...
          public function combo($id,$descripcion,$estate=true,$condition=array(),$seleccione = false,$ancho = 0){
              ...
              if ($seleccione == true)
			   ...
			   $select="<select class='ui-widget-content' id='$id' name='$id' style='width: 100%'  title='".$this->tablename."'><option value=''>[ SELECCIONE ] </option>";
    	...
    	public function getmodulo($modulo){
    		$sth=ORMConnection::Execute("SELECT modulos.m_id FROM seguridad.modulos WHERE m_descripcion='".$modulo."'");
    		foreach($sth as $id){
               // $cod=$id[0];
                $cod=$sth[0]['m_id'];
            }
    	...
    	public function session($id){
       		//$sql="SELECT * FROM permisos WHERE perfil_id='". $_SESSION['idperfil']."' and modulos_id={$this->id}";
       		$sth=ORMConnection::Execute("SELECT * FROM seguridad.permiso  WHERE idperfil='". $_SESSION['id_perfil']."' AND m_id=$id");
			//print_r($sth);
			foreach($sth as $row){
				//print_r($row);
				$_SESSION['editar']=$row['pm_modificar'];
				$_SESSION['delete']=$row['pm_eliminar'];
				$_SESSION['insertar']=$row['pm_insertar'];
				$_SESSION['acceder']=$row['pm_acceder'];
			}
      	}	 
